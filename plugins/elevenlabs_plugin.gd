...
# Cheap probe: GET /v1/user. Free, returns 200 + account info when
# the api_key is valid, 401 otherwise. Used by the credential
# editor's Test button (ADR 022).
func test_connection() -> Dictionary:
	...
	var resp: Dictionary = await _http_request(
		BASE_URL + "/user", headers, HTTPClient.METHOD_GET, "")
